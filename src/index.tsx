/**
 * This is the entry file of the Direflow setup.
 *
 * You can add any additional functionality here.
 * For example, this is a good place to hook into your
 * Web Component once it's mounted on the DOM.
 *
 * !This file cannot be removed.
 * It can be left blank if not needed.
 */

// import '@vizia/vizia-css/build/vizia-style.min.css';

import App from './components/direflow-example';
import Button from './components/button';

App.then((element) => {
    console.log('component App is mounted on the DOM', element);
});

Button.then((element) => {
    console.log('component Button is mounted on the DOM', element);
});
